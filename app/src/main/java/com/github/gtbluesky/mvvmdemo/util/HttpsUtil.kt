...
object HttpsUtil {
    ...
    fun getSslSocketFactory(trustManager: X509TrustManager): SSLParams {
        return getSslSocketFactoryBase(trustManager)
    }
    ...
    fun getSslSocketFactory(certificates: Array<InputStream>): SSLParams {
        return getSslSocketFactoryBase(certificates = certificates)
    }
    ...
    private fun getSslSocketFactoryBase(
        ...
        return try {
            ...
            manager =
                trustManager //优先使用用户自定义的TrustManager
                    ?: (trustManagers?.let {
                        chooseTrustManager(it) //然后使用默认的TrustManager
